...
<div class="row">
    <div class="col-12">
        <div class="card">
            @if($viewType == 'create')
                <form action="{{route('panel.product.image.store', $product->id)}}" method="post" enctype="multipart/form-data">
            @elseif($viewType == 'edit')
                <form action="{{route('panel.product.image.update', [$product->id, $image->id])}}" method="post" enctype="multipart/form-data">
                @method('patch')
            @endif
            @csrf
                ...
                <div class="form-group row mb-2">
                    ...
                    <div class="col-sm-10">
                        ...
                        <div class="mb-2 border" style="width: 200px">
                            <img src="{{asset('storage/'. $image->image)}}" class="img-fluid" style="object-fit: contain;" alt="user photo">
                        </div> 
                        @endisset
                ...
                <div class="form-group row mb-2">
                    ...
                    <div class="col-sm-10">
                        <select name="status" class="form-control" {{$attr}}>
                            ...
                            <option value="{{ $key }}" {{isset($image->status) && $key == $image->status ? 'selected' : ''}}>{{ $item }}</option>
                            @endforeach
                        </select>
                        @error('status')
                            <span class="text-danger ms-1">{{ $message }}</span>
            ...
            <div class="card-footer">
                ...
                @elseif ($viewType == 'show')
                    <a href="{{route('panel.product.image.edit', [$product->id, $image->id])}}" class="mb-2 mr-2 btn btn-primary"
                        title="Back">
