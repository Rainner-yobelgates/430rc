...
<div class="row">
    <div class="col-12">
        <div class="card">
            @if($viewType == 'create')
                <form action="{{route('panel.color.store')}}" method="post" enctype="multipart/form-data">
            @elseif($viewType == 'edit')
                <form action="{{route('panel.color.update', $color->id)}}" method="post" enctype="multipart/form-data">
                @method('patch')
            @endif
            @csrf
            ...
            <div class="card-footer">
                ...
                @elseif ($viewType == 'show')
                    <a href="{{route('panel.color.edit', $color->id)}}" class="mb-2 mr-2 btn btn-primary"
                        title="Back">
                        <i class="fas fa-edit"></i><span> Edit</span>
                    </a>
                @endif
                <a href="{{route('panel.color.index')}}" class="mb-2 mr-2 btn btn-warning"
                   title="Back">
